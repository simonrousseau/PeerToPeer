...
public class Peer {
    ...
    private String ipServeur = "172.21.65.190";
    ...
    public void majTable(){
        try{
            for(int i = 1; i <= 6;i++){
                ...
                if(k <= this.nextHash){
                    this.tableRoutage[i-1].setIp(this.nextIp);
                    this.tableRoutage[i-1].setHash(this.nextHash);
                }
                else{
                    int k_recalculer = k;
                    if (k > 100){
    ...
    public LigneTable recupererSuccesseur(int k) throws UnknownHostException, IOException{
        ...
        return new LigneTable(reponseFormer[1], Integer.parseInt(reponseFormer[0]));
    }
    ...
    public class PeerRunnable implements Runnable{
        ...
        public void rechercheSuccesseur(String[] words) throws UnknownHostException, IOException{
            ...
            if((pair.monHash < hashTest && pair.nextHash > hashTest) || (pair.nextHash < pair.monHash && (pair.nextHash > hashTest || pair.monHash < hashTest))){
